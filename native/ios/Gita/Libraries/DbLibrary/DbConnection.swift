...
//This is custom LIKE sql operator. The point is to make search valid for Cyrillic text & case insensitive.
//By default it's case sensitive and you need to modify actual sql table to enable case insensitive search.
//NOTE: It is implemented as contains and does not support parameters and wildcards
//Usage: LIKE 'filterString'
fileprivate func nocaseUtf8Like(_ ctx: OpaquePointer?, _ argc: Int32, _ argv: UnsafeMutablePointer<OpaquePointer?>?) {
	let SQLITE_STATIC = unsafeBitCast(0, to: sqlite3_destructor_type.self)
	let SQLITE_TRANSIENT = unsafeBitCast(-1, to: sqlite3_destructor_type.self)
	
	guard let argv0 = argv?[0] else {
		return
	}
	
//	print("LIKE argc: \(argc)")
	
	switch sqlite3_value_type(argv0) {
	case SQLITE_NULL:
		sqlite3_result_text(ctx, "NULL", 4, SQLITE_STATIC)
		break
	case SQLITE_TEXT:
		//TODO: handle parameters + wildcards (%, _)
		let s1 = String(cString: sqlite3_value_text(argv0))
		let s2 = String(cString: sqlite3_value_text(argv?[1]))
		
//		print("Arguments: \(s1), \(s2)")
		if s2.count >= s1.count {
			let range = s2.range(of: s1, options: .caseInsensitive)
			let res: Int32 = range != nil ? 1 : 0
			sqlite3_result_int(ctx, res)
		} else {
			sqlite3_result_int(ctx, 0)
		}
		break
		//Strange call with int argument
//	case SQLITE_INTEGER:
//		print("integer")
//		break
	default:
...
class DbConnection {
	...
	func open() {
		if sqlite3_open(self.dbPath, &database) == SQLITE_OK {
//			print("DB opened")
			setNocaseForUTF8()
		} else {
...
//			print("DB closed")
	...
	*/
	...
	func setNocaseForUTF8() {
		//UnsafeRawPointer(Unmanaged<AnyObject>.passUnretained(nocaseLike as AnyObject).toOpaque())
		var res = sqlite3_create_function(self.database, "LIKE", 3, SQLITE_ANY, nil, nocaseUtf8Like, nil, nil)
		if res != SQLITE_OK {
			print("Error creating custom LIKE function: \(res)")
		}
		res = sqlite3_create_function(self.database, "LIKE", 2, SQLITE_ANY, nil, nocaseUtf8Like, nil, nil)
		if res != SQLITE_OK {
			print("Error creating custom LIKE function: \(res)")
		}
	}
